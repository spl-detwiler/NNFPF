...
% velocity distribution is solved, using a mixed Darcy-Cubic law
% formulation that employs a Geometric mean to calculated the permeability
% at each node based on the solid content. (Building block to the
% Ellis_Artificial_Removal Code
%
%
% INPUT			DESCRIPTION
...
h=zeros(nx-2,ny-2).*linspace( hin-(0.05*hin),hout+(0.05*hout),ny-2 );
...
SAVENAME = [phiNAME(5:18) sprintf('_hin_%1.2f_.mat',hin)];
save(SAVENAME,'phi','h','qx','qy','RES');
